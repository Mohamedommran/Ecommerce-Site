var cart = {
  ...
  init : () => {
    ...
    cart.hItems = document.getElementById("modalBody");
    // (C2) DRAW PRODUCTS LIST
    cart.hPdt.innerHTML = "";
    let p, item, part;
    for (let id in products) {
      // Get the products by it's id
      p = products[id];
      ...
      part.onclick = () => { 
        alert('you added '+ products[id].name +' to your cart');
        //setTimeout(function) not done yet
        cart.add(id);
       };
      item.appendChild(part);
    }
  ...
  // (H) CHECKOUT
  checkout : () => {
    alert("it doesn*t work yet, come back later ;)");
  }
};
